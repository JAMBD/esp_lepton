...
# the PATH.
XTENSA_TOOLS_ROOT ?= $(ROOT_DIR)/esp-open-sdk/xtensa-lx106-elf/bin/

# base directory of the ESP8266 SDK package, absolute
SDK_BASE	?= $(ROOT_DIR)/esp-open-sdk/sdk/
...
ESPTOOL		?= PATH=$(PATH):$(XTENSA_TOOLS_ROOT) esptool.py
ESPPORT		?= /dev/ttyUSB0
...
CFLAGS += -DUSER_MAIN_SSID='"$(USER_MAIN_SSID)"' -DUSER_MAIN_WPA2PW='"$(USER_MAIN_WPA2PW)"'
...
ESPTOOL_FLASHDEF=
#--flash_freq $(ESPTOOL_FREQ) --flash_mode $(ESPTOOL_MODE) --flash_size $(ESPTOOL_SIZE)
...
.PHONY: all checkdirs clean esp-open-sdk libesphttpd default-tgt
...
esp-open-sdk: esp-open-sdk/Makefile
	$(Q) cd esp-open-sdk; make

libesphttpd: libesphttpd/Makefile
	$(Q) make -C libesphttpd USE_OPENSDK=$(USE_OPENSDK) XTENSA_TOOLS_ROOT=$(XTENSA_TOOLS_ROOT) SDK_BASE=$(SDK_BASE)

$(APP_AR): esp-open-sdk libesphttpd $(OBJ)
	$(vecho) "AR $@"
	$(Q) $(AR) cru $@ $(OBJ)
